...
新应用 **必须** 遵守平台设计系统（详见 `docs/design_system.md`）：

1. 页面使用 `ui-page`、`ui-card`、`ui-form-field` 等全局注册的公共组件构建 UI。
2. 颜色使用 `var(--color-*)` Token，不得自定义平台主色、按钮、卡片、状态标签。
3. 页面 wxss 仅保留业务私有样式（如结果展示区布局）。
4. 模板已预配置设计系统引用，复制即可使用。
...
- **禁止** 应用页面重新定义平台主色（`--color-primary`）、卡片样式、按钮样式、状态标签样式。
- **禁止** 应用页面内联硬编码 `#07c160`、`#1890ff`、`#52c41a` 等非 Token 颜色值（微信品牌色仅用于微信登录按钮）。
- **禁止** 应用页面不使用 `ui-page` 等公共组件，自行搭建独立页面壳。
